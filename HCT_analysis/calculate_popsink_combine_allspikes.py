...
def calculate_popsink_combine_allspikes(derivatives_base, unit_type, load_units_which_method = [1,2,3], title='Population Sinks', dir = 'popsinks', goals_to_include = [0,1,2], frame_rate=25,
                      sample_rate=30000, code_to_run=[1,2]):
    """
    calculates the population sink for the whole trial
    NOTE: here the popsink is calculated by just combining all of the spikes of all of the units
    """
    if unit_type not in ['pyramidal', 'good', 'all']:
        raise ValueError('unit type not correctly defined')

    # Load directories and data
    rawsession_folder, sorting, pos_data, pos_data_iv, output_folder= load_directories(derivatives_base, dir)
    ...
    ncols = len(load_units_which_method)
    nrows = len(goals_to_include)
    ...
    if 1 in code_to_run:
        ...
        for i_g, g in enumerate(goals_to_include):
            ...
            for val, unit_id in tqdm(enumerate(unit_ids)):
                ...
                ind_nan = np.where(np.isnan(scaled_vecs))[0]
                #scaled_vecs[ind_nan] = [0,0] ## CAHNGE FOR NAN NO NAN
...
if __name__ == "__main__":
    ...
    calculate_popsink(derivatives_base, title = 'Population sink sub 002 ses 02 11/09', unit_type='pyramidal', dir = 'popsinks', code_to_run=[1,2],
