...
def find_consink(spike_train, reldir_occ_by_pos, sink_bins, direction_bins, candidate_sinks, pos_data,
    ...
    rel_dir_ctrl_dist, n_spikes_total = rel_dir_ctrl_distribution_all_sinks(spike_train, reldir_occ_by_pos, sink_bins,
                                                                            candidate_sinks, pos_data)
...
def shift_spiketrain_pergoal(spike_train, goal, intervals_frames, n_frames: int, frame_rate=25):
    ...
    lengths = [intervals_frames.iloc[tr, start_col + 1] - intervals_frames.iloc[tr, start_col] for tr in
...
def calculate_translated_mrl(spiketrain, dlc_data, reldir_occ_by_pos, sink_bins, direction_bins, candidate_sinks,
                             reldir_allframes, rawsession_folder, intervals_frames, goal):
    n_frames = len(dlc_data)
    translated_spiketrain = shift_spiketrain_pergoal(spiketrain, goal, intervals_frames, n_frames)
    mrl, _, _ = find_consink(translated_spiketrain, reldir_occ_by_pos, sink_bins, direction_bins, candidate_sinks,
                             dlc_data, reldir_allframes)
    return mrl


def recalculate_consink_to_all_candidates_from_translation(spiketrain, dlc_data, reldir_occ_by_pos, sink_bins,
                                                           direction_bins, candidate_sinks, reldir_allframes,
                                                           rawsession_folder, intervals_frames, goal):
    n_shuffles = 1000
    ...
    mrl = Parallel(n_jobs=-1, verbose=0)(
        delayed(calculate_translated_mrl)(spiketrain, dlc_data, reldir_occ_by_pos, sink_bins, direction_bins,
                                          candidate_sinks, reldir_allframes, rawsession_folder, intervals_frames, goal)
...
def main(derivatives_base, rel_dir_occ: Literal['all trials', 'intervals'],
         unit_type: Literal['pyramidal', 'good', 'all'], code_to_run=[], frame_rate=25, sample_rate=30000):
    """
    ...
    if -1 in code_to_run:
        ...
        reldir_occ_by_pos, sink_bins, candidate_sinks = get_relative_direction_occupancy_by_position(pos_data_reldir,
                                                                                                     limits)
        ...
        for unit_id in tqdm(unit_ids):
            for g in [0, 1, 2]:
                ...
                ci = recalculate_consink_to_all_candidates_from_translation(spike_train, pos_data,
                                                                            reldir_occ_by_pos_cur, sink_bins,
                                                                            direction_bins, candidate_sinks,
                                                                            reldir_allframes, rawsession_folder,
                                                                            intervals_frames, goal=g)
    ...
    plot_dir = os.path.join(derivatives_base, 'analysis', 'cell_characteristics', 'spatial_features',
                            'consink_plots_newmethod')
    ...
    plot_all_consinks(consinks_df, goal_coordinates, hcoord, vcoord, limits, jitter=jitter, plot_dir=plot_dir,
                      plot_name='ConSinks Good Units')
