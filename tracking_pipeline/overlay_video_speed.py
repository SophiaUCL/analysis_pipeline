...
def overlay_video_speed(derivatives_base: Path, trials_to_include: list, short: bool = False):
    """
    Overlays videos with the position of the animal and a dot thats blue if speed > threshold and red if below
    ...
    for tr in trials_to_include:
        ...
        # Path for output
        output_path = output_dir/f"t{tr}_with_speed.avi"
...
def do_overlay(video_path: Path, df: pd.DataFrame, output_path: Path, short: bool):
    ...
    while cap.isOpened():
        ...
        if short and frame_idx > 25*30:
            break
        if frame_idx < len(head_pos_x_col) and pd.notna(head_pos_x_col[frame_idx]) and pd.notna(head_pos_y_col[frame_idx]) and pd.notna(speed_col[frame_idx]):
            x = int(head_pos_x_col[frame_idx])
            y = int(head_pos_y_col[frame_idx])
            speed = speed_col[frame_idx]

            # Draw a red square of 5x5 pixels at the head_pos
            size = 5
            if speed > 0:
                color = (255, 0, 0) # blue if speed 
            else:
                color = (0, 0, 255) # red if not speed 
...
if __name__ == "__main__":
    ...
    overlay_video_speed(derivatives_base, [2], False)
