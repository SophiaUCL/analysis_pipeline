...
Methods = Literal["image", "video"]


def get_limits(derivatives_base: Path):
    """
    ...
    # Output folder
    output_path = derivatives_base/"analysis"/"maze_overlay"/"limits.png"
    ...
    while not good_limits:
        ...
        if clicked: 
            ...
            break
        ax.set_title("Click to define new limits. Press ESC to exit")
...
def get_image(rawsession_folder: Path, method: Methods = "video"): 
...
def save_limits(xmin: float, xmax: float, ymin: float, ymax: float, derivatives_base: Path):
...
def plot_sink_bins(derivatives_base: Path, img: np.ndarray = None, method = "video", sink_bins = None):
