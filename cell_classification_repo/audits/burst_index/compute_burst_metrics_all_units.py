...
def _load_cluster_groups(derivatives_base: str) -> Dict[int, str]:
    """
    Read Phy curation cluster_group.tsv and return dict: {cluster_id: group}.
    group is typically one of: good / mua / noise (but may vary).
    """
    ...
    return {int(cid): str(grp) for cid, grp in zip(df["cluster_id"].values, df["group"].values)}
...
def burst_contrast_from_acg(bins_ms: np.ndarray, counts: np.ndarray) -> float:
    """Legacy/shape-sensitive burst contrast metric (robust, warning-free)."""
    ...
    max_peak_10 = float(np.max(a))
    mean_val_40_50 = float(np.mean(b))
    if max_peak_10 == 0 or (not np.isfinite(mean_val_40_50)):
        return np.nan
...
def burst_index_paper_from_acg(
    ...
    num = float(np.sum(cou[win_0_10]))
    den = float(np.sum(cou[win_40_50]))
    if den <= 0:
...
def compute_burst_metrics_all_units(
    ...
    # NEW: phy group for each cluster
    groups = _load_cluster_groups(derivatives_base)

    unit_ids = [int(u) for u in sorting.unit_ids]
    print(f"ALL units: {len(unit_ids)}")

    # ---- OUTPUT (write here) ----
    out_dir = Path(project_base) / "audits" / "burst_index"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_csv = out_dir / "burst_metrics_all_units.csv"
    ...
    for uid in unit_ids:
        ...
        frac2, nviol2, nspk = frac_spikes_with_next_isi_below(st_s, threshold_ms=qc_threshold_ms)
        paper_excl = bool(np.isfinite(frac2) and (frac2 > qc_exclude_frac))

        bins_ms, counts = autocorrelogram_histogram(
            st_s, bin_ms=bin_ms, max_lag_ms=max_lag_ms, positive_lags_only=positive_lags_only
        )
        ...
        rows.append(
            dict(
                unit_id=int(uid),
                phy_group=groups.get(int(uid), "unlabeled"),
                n_spikes=int(nspk),
...
if __name__ == "__main__":
    ...
    compute_burst_metrics_all_units(
